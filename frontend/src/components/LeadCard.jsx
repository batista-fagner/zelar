...
const labelColor = {
  ...
  buscando_cuidador: 'bg-blue-100 text-blue-600',
  broadcast_expirado: 'bg-yellow-100 text-yellow-700',
  cuidador_designado: 'bg-emerald-100 text-emerald-700',
}
...
export default function LeadCard({ lead, onClick, onDelete, onLeadUpdate }) {
  ...
  async function handleRebroadcast() {
    ...
    try {
      const res = await fetch(`${API_URL}/leads/${lead.id}/rebroadcast`, { method: 'POST' })
      const data = await res.json()
      if (data.ok) await handleOpenBroadcast()
      else console.error('Erro ao reenviar broadcast:', data.error)
    } catch (err) {
      console.error('Erro ao reenviar broadcast:', err)
    } finally {
  ...
  async function handleCompleteCourse() {
    ...
    try {
      const res = await fetch(`${API_URL}/leads/${lead.id}/complete-course`, { method: 'POST' })
      const data = await res.json()
      if (data.ok) onLeadUpdate?.({ ...lead, courseSurveySentAt: new Date().toISOString() })
    } catch (err) {
      console.error('Erro ao marcar curso concluído:', err)
    } finally {
  ...
  return (
    ...
    >
      ...
      {/* Botão marcar curso concluído (dispara pesquisa de satisfação) */}
      {lead.stage === 'matriculado' && !lead.courseSurveySentAt && (
        <div className="mt-2 pt-2 border-t border-emerald-100">
          <button
            onPointerDown={(e) => e.stopPropagation()}
            onClick={(e) => { e.stopPropagation(); setShowCompleteCourseModal(true) }}
            disabled={completingCourse}
            className="w-full flex items-center justify-center gap-1.5 text-[11px] font-semibold px-2 py-1.5 rounded-lg bg-emerald-500 hover:bg-emerald-600 text-white transition-colors disabled:opacity-50"
          >
            <CheckCircle className="w-3.5 h-3.5" />
            {completingCourse ? 'Enviando...' : 'Marcar Curso Concluído'}
          </button>
        </div>
      )}
        ...
        >
          <div className="bg-white rounded-xl shadow-xl p-6 w-72 space-y-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-sm font-semibold text-gray-800">Marcar curso como concluído?</h3>
                <p className="text-xs text-gray-500 mt-0.5">Vamos enviar a pesquisa de satisfação do curso para <span className="font-medium">{lead.name || lead.phone}</span> agora.</p>
              </div>
            </div>
            <div className="flex gap-2">
              <button
                onClick={() => setShowCompleteCourseModal(false)}
                className="flex-1 py-2 text-sm text-gray-600 border border-gray-200 rounded-lg hover:bg-gray-50 transition"
              >
                Cancelar
              </button>
              <button
                ...
                className="flex-1 py-2 text-sm font-medium text-white bg-emerald-500 hover:bg-emerald-600 rounded-lg transition"
              >
                Confirmar
              </button>
            </div>
            ...
            {loadingBroadcast ? (
              ...
              <>
                {broadcastData.summary && (
                  <div className="bg-slate-50 border border-slate-200 rounded-lg px-2.5 py-2 space-y-1">
                    <p className="text-[11px] font-semibold text-slate-600">Resumo do atendimento</p>
                    {SUMMARY_FIELDS.filter(([key]) => broadcastData.summary[key]).map(([key, label]) => (
                      <p key={key} className="text-[11px] text-slate-600">
                        <span className="font-medium">{label}:</span> {broadcastData.summary[key]}
                      </p>
                    ))}
                    {broadcastData.complexity && (
                      <p className="text-[11px] text-slate-600">
                        <span className="font-medium">Complexidade:</span> {broadcastData.complexity}
                      </p>
                    )}
                  </div>
                )}
                ...
                {['expirado', 'cancelado'].includes(broadcastData.status) && (
                  <button
                    onClick={handleRebroadcast}
                    disabled={rebroadcasting}
                    className="w-full flex items-center justify-center gap-1.5 text-[11px] font-semibold px-2 py-1.5 rounded-lg bg-blue-500 hover:bg-blue-600 text-white transition-colors disabled:opacity-50"
                  >
                    <Send className="w-3.5 h-3.5" />
                    {rebroadcasting ? 'Reenviando...' : 'Reenviar aos cuidadores'}
                  </button>
                )}
              </>
            )}
